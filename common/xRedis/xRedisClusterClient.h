...
public:
	...
	bool ConnectRedis(const char *host, uint32_t port, const char *pswd, uint32_t poolsize);
	bool ReConnectRedis();
    bool ReConnectRedis(RedisConnection *pConn);
    void Keepalive();
	bool ClusterEnabled()const{ return mClusterEnabled; }
    bool RedisCommandArgv(const VSTRING& vDataIn, RedisResult &result);

	/* format 格式必须为： "[get|set|hget|...] %s %a %b %c ..."，第一个%s就是要操作的redis的那个key
	   也就是说，format后的那个不定参数列表，第一个必须是要操作的redis key
	 */
