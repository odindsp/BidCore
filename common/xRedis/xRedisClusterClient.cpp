...
{   
	...
	return ConnectRedis(pConn->mHost, pConn->mPort, pConn->mPswd.c_str(), pConn->mPoolSize);
}

bool xRedisClusterClient::ReConnectRedis()
{
	RedisConnection *pConn = GetConnection(0);
	if (!pConn){
		return false;
	}

	string host = pConn->mHost;
	string pswd = pConn->mPswd;
	uint32_t port = pConn->mPort;
	uint32_t poolSize = pConn->mPoolSize;
	Release();
	return ConnectRedis(host.c_str(), port, pswd.c_str(), poolSize);
}

bool xRedisClusterClient::ConnectRedis(const char *host, uint32_t port, const char *pswd, uint32_t poolsize)
{
    if (NULL == host) {
        printf("error argv \r\n");
        return false;
	...
	if (!mPswd.empty())
	{
		redis_reply = (redisReply*)redisCommand(redis_ctx, "AUTH %s", pswd);
		if ((NULL == redis_reply) || (NULL == redis_reply->str) || strcasecmp(redis_reply->str, "OK") != 0)
		{
			if (redis_reply) {
				freeReplyObject(redis_reply);
			}
			redisFree(redis_ctx);
			printf("redis cluster [%s:%u] password err!\n", host, port);
			return false;
		}
	}

	redis_reply = (redisReply*)redisCommand(redis_ctx, "PING");
	if ((NULL == redis_reply) || (NULL == redis_reply->str) || strcasecmp(redis_reply->str, "PONG") != 0)
	{
		if (redis_reply) 
		{
    ...
    vector<string> vlines;
    redis_reply = (redisReply*)redisCommand(redis_ctx, "CLUSTER NODES");
    if( (NULL==redis_reply) ||(NULL==redis_reply->str)) {
        if(redis_reply) {
            freeReplyObject(redis_reply);
        }
    ...
    try
    {
        ...
        for (uint32_t i = 0; i < poolsize; ++i)
        {
            ...
            if (pRedisconn->mCtx == NULL || pRedisconn->mCtx->err) {
				...
				redisCommand(pRedisconn->mCtx, "AUTH %s", mPswd.c_str());
...
bool xRedisClusterClient::RedisCommand(RedisResult &result, const char *format, ...)
{
   ...
   // printf("key:%p \n", key);
    ...
    if (CheckReply(reply)) {
        ...
        //printf("%d %lld %d %s %lu \r\n", 
        //    reply->type, reply->integer, (int)reply->len, reply->str, reply->elements);
